...
from pp_webkitplayer import WebKitPlayer
...
class Show(object):
    ...
    def base__init__(self,
        ...
        self.show_manager.pre_init(self.show_id,self.showlist,self.show_params,self.root,self.pp_dir,self.pp_profile,self.pp_home)
    ...
    def base_init_selected_player(self,selected_track):
        ...
        if track_type == "image":
            ...
            self.mon.err(self,'mplayer Audio Player is not implemented, use MPV Player')
            return None

        #gtkdo art might work
        #elif track_type == "webkit" and self.show_params['type'] not in ('artmediashow','artliveshow'):
        elif track_type == "webkit":
            return WebKitPlayer(self.show_id,self.showlist,self.root,self.show_canvas,
                                 self.show_params,selected_track,self.pp_dir,self.pp_home,
                                 self.pp_profile,self.end,self.command_callback)
    ...
    def base_close_or_unload(self):
        ...
        if self.current_player is not None:
            ...
            self.wait_timer=GLib.timeout_add(1,self._wait_for_end)
            return
        else:
    ...
    def _wait_for_end(self):
        if self.wait_timer!=None:
            GLib.source_remove(self.wait_timer)
            self.wait_timer=None
        if self.leak is True:
            print('IN ',self.show_params['title'])
        self.mon.trace(self, self.mon.pretty_inst(self.current_player))
        if self.current_player is not None:
            ...
            if self.current_player.play_state not in ('unloaded','closed','load-failed'):  ####
                #self.canvas.xafter(50,self._wait_for_end)
                self.wait_timer=GLib.timeout_add(50,self._wait_for_end)
            else:
  ...
  # respond to input events
    ...
    def base_load_show_background(self):
        ...
        if self.background_file != '':
            background_img_path = self.base_complete_path(self.background_file)
            if not os.path.exists(background_img_path):
                return 'error',"Show background file not found "+ background_img_path

            #print ('make show background image',self.background_file,background_img_path)
            self.show_background_obj=Gtk.Picture.new_for_filename(background_img_path)
            self.show_background_obj.set_size_request(self.show_canvas_width,self.show_canvas_height)
            self.show_background_obj.set_content_fit(Gtk.ContentFit.FILL)
            self.canvas.put(self.show_background_obj,self.show_canvas_x1,self.show_canvas_y1)
            if self.show_background_obj != None:
                self.show_background_obj.set_visible(False)
    ...
    def base_show_show_background(self):
        if self.show_background_obj is not None:
            # print 'show show background'
            self.show_background_obj.set_visible(True)
    ...
    def base_delete_show_background(self):
        if self.show_background_obj is not None:
            ...
            self.canvas.remove(self.show_background_obj)
...
# *********************************
    ...
    def display_eggtimer(self):
        ...
        if text != '':
            self.egg_timer=Gtk.Label()
            self.egg_timer.set_label(text)
            self.egg_timer.set_name('egg-timer')
            self.egg_timer.set_justify(CSS.justify_map[self.show_params['eggtimer-justify']])
            #gtkdo where does justify come from
            self.css.style_widget(self.egg_timer,'egg-timer',color=self.show_params['eggtimer-colour'],font=self.show_params['eggtimer-font'])
            x,y,=calculate_text_position(self.show_params['eggtimer-x'],self.show_params['eggtimer-y'],
                                     self.show_canvas_x1,self.show_canvas_y1,
                                     self.show_canvas_width,self.show_canvas_height,self.egg_timer)
            self.canvas.put(self.egg_timer,x,y)
    ...
    def delete_eggtimer(self):
        if self.egg_timer is not None:
            self.egg_timer.set_visible(False)
            self.egg_timer=None
...
# *********************************
    ...
    def display_admin_message(self,text):
        ...
        self.admin_message=Gtk.Label()
        self.admin_message.set_label(text)
        self.admin_message.set_justify(CSS.justify_map[self.show_params['admin-justify']])
        self.admin_message.set_name('admin-message')
        self.css.style_widget(self.admin_message,'admin-message',color = self.show_params['admin-colour'],font = self.show_params['admin-font'])
        x,y=calculate_text_position(self.show_params['admin-x'],self.show_params['admin-y'],
                             self.show_canvas_x1,self.show_canvas_y1,                                    
                             self.show_canvas_width,self.show_canvas_height,self.admin_message)
        self.canvas.put(self.admin_message,x,y)
    ...
    def delete_admin_message(self):
        if self.admin_message is not None:
            self.canvas.remove(self.admin_message)
            self.admin_message=None
