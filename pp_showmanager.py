...
class ShowManager(object):
   ...
   However a workaround is Fto make the secong instance a subshow of a mediashow with a different reference.
...
# *********************************
    ...
    def pre_init(self,show_id,showlist,show_params,root,pp_dir,pp_profile,pp_home):
        ...
        #self.starter_show=starter_show
    ...
    def wait_for_openexclusive(self):
        if self.all_shows_exited() is False:
            GLib.timeout_add(1,self.wait_for_openexclusive)
            return False
    ...
    def compute_show_canvas(self,show_params):
        canvas={}
        status,message,self.display_name = self.dm.is_display_connected(show_params['display-name'])
        if status != 'normal':
            return 'error', message,None
        canvas['canvas-obj']=self.dm.get_canvas_obj(self.display_name)       
        status,message,self.show_canvas_x1,self.show_canvas_y1,self.show_canvas_width,self.show_canvas_height= self.parse_show_canvas(show_params['show-canvas'])
        if status  == 'error':
        ...
        self.show_canvas_x2 = self.show_canvas_x1 + self.show_canvas_width
        self.show_canvas_y2=self.show_canvas_y1 + self.show_canvas_height
        ...
        canvas['display-name']=self.display_name
    ...
    def parse_show_canvas(self,text):
        ...
        display_width,display_height=self.dm.get_canvas_dimensions(self.display_name)
        if len(fields) < 1:
            #get canvas dimensions from the display manager
            return 'normal','',0,0,display_width,display_height
             
        elif len(fields) == 1:
            # window is specified
            status,message,x,y,canvas_width,canvas_height=parse_rectangle(text)
            if status=='error':
                return 'error',message,0,0,0,0
            elif x==-1 and y==-1:
                x=(display_width-canvas_width)/2
                y=(display_height-canvas_height)/2
                return 'normal','',x,y,canvas_width,canvas_height
            else:
                return 'normal','',x,y,canvas_width,canvas_height
        else:
            # error
